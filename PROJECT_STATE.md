...
## Backend — Fase 5C UI5/Electron completada local (pendiente PR `feature/fase5c-ui-electron` → `develop`) — incluye Fase 5B

- [x] Fase 5C — Wiring hexagonal `RagPort`/`PromptPort` a HTTP + IPC (2026-09-03): `packages/backend/src/rag/routes.ts` `createRagRouter` (`GET /rag/search?q&limit&accountId`, valida `q` required, `limit` 1..50, DI `RagPort.search`, spread condicional `exactOptionalPropertyTypes`) + `packages/backend/src/prompts/routes.ts` `createPromptRouter` (`GET /prompts`, `GET /prompts/:name?version`, `POST /prompts/render` `{name,variables,version}`, `POST /prompts/evaluate` `{cases}`, `AppError` mapping); `packages/backend/src/app.ts` extiende `CreateAppOptions` con `rag?`/`prompts?` DI y monta `/rag`+`/prompts`; `packages/backend/src/index.ts` re-exporta routers; 10 tests `routes.test.ts` TDD `vi.fn`+`supertest`, 212 tests 91.15%/83.93% verde, `lint`/`format:check`/`typecheck`/`build` OK

## Backend — Fase 5B prompts completada local (merge pendiente vía 5C)
...
- [x] Fase 5C — UI5 wiring `RagPort`/`PromptPort` (2026-09-03, `sapui5-email-ia`): `webapp/service/EmailService.js` añade `searchRag`/`listPrompts`/`getPrompt`/`renderPrompt`/`evaluatePrompts` vía `fetch` `baseUrl` DI sin deps; `view/Inbox.view.xml` `SearchField` + `Button` + `List` `ragResults` `app>/ragResults` + `view/Home.view.xml` `List` `app>/prompts` + `Button` refresh; `controller/Inbox` `onSearchRag` `MessageToast`, `controller/Home` `onInit/onRefreshPrompts` DI `EmailService`; `i18n` `ragSearchPlaceholder`/`ragSearchButton`/`ragResultsHeader`/`ragNoResults`/`promptListHeader`/`promptListEmpty`/`promptRefresh` en 3 locales; `ui5 build --all` 35 s OK, sin nueva dep (ponytail)
...
- [x] Fase 2 completada (ADR-006, 2026-08-22): `vite@6.3.5` + `vite-plugin-electron@0.28.8` (HMR) + `vite.config.ts` (main `src/main.ts` → `dist/main.js`, preload `src/preload.ts` → `dist/preload.mjs` + `dist-electron/preload.mjs`, renderer `dist/index.html`), `electron-builder.yml` (appId `com.emailia.app`, files `dist/**/*` + `preload.cjs`/`index.html`), IPC tipado (`ipc.ts` + `preload.ts`/`preload.cjs` expone `ping`/`getVersions` vía `ipcMain.handle`/`ipcRenderer.invoke`), `pnpm --filter @electron dev/build:vite/package` OK; `pnpm build` (tsc) + `pnpm typecheck` verdes; empaquetado vía `npx electron-builder` (paquete no instalado en workspace por hang pnpm en Windows — ver Hallazgos)
- [x] Fase 5C — IPC wiring `RagPort`/`PromptPort` (2026-09-03, `hexagonal-email-ia`): `packages/electron/src/ipc.ts` `IpcDeps` DI `rag?`/`prompts?` + handlers `rag:search`/`prompts:list`/`prompts:get`/`prompts:render`/`prompts:evaluate` vía `ipcMain.handle`, preserva `ping`/`getVersions`; `src/preload.ts` + `preload.cjs` exponen `searchRag`/`listPrompts`/`getPrompt`/`renderPrompt`/`evaluatePrompts` vía `contextBridge` `sandbox:true` (CJS para fallback); DI mantiene dependency rule `shared←core←backend←electron` (`electron` consume ports, no implementa)
...
- [x] Fase 4 — Contratos formales Pact + MSW + OAuth/token + sync incremental offline-first (§3.7) (2026-08-27): `TokenProvider` Bearer + `EmailSyncService` incremental + MSW `createHandlers` + Pact V3 4 interacciones (137 tests, 93.24%/85.74%) **mergeado** en `develop` PR #16 `f072dc1` (`quality`+CodeQL SUCCESS)
...
- [x] Fase 5B — prompts versionados + golden dataset (ADR-010, 2026-08-28): `PromptPort`+`PromptError` en `core` + `PromptService` (`get`/`list`/`render`/`evaluate`) en `backend/src/prompts/` + recursos JSON `summarize-email v1.0.0/1.1.0` + `classify-email v1.0.0` + `golden-dataset.json` 3 casos; DI `AIProviderPort` mockeada, semver ordering, variables tipadas `string|number|boolean`; 202 tests 91.82%/83.94% verde local (incluido en `feature/fase5c-ui-electron`, pendiente merge PR #28)
- [x] Fase 5C — UI5/Electron `RagPort`/`PromptPort` wiring (2026-09-03): Backend `GET /rag/search` + `GET /prompts*` + `POST /prompts/render|evaluate` (10 tests), Frontend `sapui5-email-ia` Home/Inbox + `EmailService` + `i18n` 3 locales, Electron `hexagonal-email-ia` IPC DI `rag:search`/`prompts:*` + `preload` CJS/MJS, `createApp` DI `rag`/`prompts?`; 212 tests 91.15%/83.93% verde, `ui5 build` 35 s, `lint`/`format:check`/`typecheck`/`build` verdes, sin ADR (no decisión arquitectónica, solo wiring §3.6/§3.8, ponytail YAGNI), pendiente PR `feature/fase5c-ui-electron` → `develop`
- Futuro — Reevaluar Nx/Turborepo, Loki/Tempo o SaaS solo si hay destino de despliegue (ADR-002/009)
...
- Fase 5C UI5/Electron (2026-09-03): Backend `createRagRouter`/`createPromptRouter` DI `RagPort`/`PromptPort` + `exactOptionalPropertyTypes` spread condicional `accountId`, `createApp` DI `rag`/`prompts`; Frontend `sapui5-email-ia` `SearchField` `ragResults` + `prompt` list/refresh + `EmailService` `searchRag`/`listPrompts` fetch DI; Electron `hexagonal-email-ia` `IpcDeps` DI `rag`/`prompts` handlers `rag:search`/`prompts:*` + `preload.ts/.cjs` `sandbox:true` CJS fallback; `ui5 build` 35 s, `pnpm format` corrige 8 files (LF), coverage 212 tests 91.15%/83.93% verde (overall 80% threshold OK, `routes.ts` 95%/78.94% por ramas no cubiertas); sin ADR.
...
- Ninguno bloqueante. Fase 5C **completada local** `feature/fase5c-ui-electron` (212 tests 91.15%/83.93% verde, `lint`+`format:check`+`typecheck`+`build`+`ui5 build` verdes, `RagPort`/`PromptPort` wiring), incluye Fase 5B (202 tests, PR #28 `quality`+CodeQL SUCCESS). Fase 5A RAG **mergeado** PR #25 `6172fbe` (186 tests 91.75%/83.99% `quality`+CodeQL SUCCESS). Fase 4+ Runbooks **mergeado** PR #22 `2461a95`. OAuth real sin credenciales mitigado `TokenProvider`+`SecretStorePort`; hang pnpm+electron-builder mitigado; CodeQL cuota Free OK; OTel off por defecto; Pact V3 warning no bloqueante.

---

# Próxima tarea

- **Fase 5D — Adjuntos / vector nativo / sqlite-vec (§3.7 futuro)** — Tras Fase 5C local (212 tests 91.15%/83.93% verde). Siguiente según `ARCHITECTURE_DECISIONS:3.7`: ampliar `email_chunks` a adjuntos, evaluar `sqlite-vec` ANN vs JSON brute-force, tipar `vector` nativo Drizzle, ampliar `golden-dataset.json` con casos reales anonimizados (privacy-first). Alternativa §3.6/§3.8: OpenAPI/Scalar auto-docs si se prioriza `api-documentation` skill. Reevaluar Nx/Turborepo, Loki/Tempo o SaaS solo si hay destino de despliegue (ADR-002/009).

---

# Commits pendientes

- **Fase 5C** — commit local pendiente PR `feature/fase5c-ui-electron` → `develop` (incluye Fase 5B `89bddf1`): `feat(backend): Rag/Prompt HTTP routers GET /rag/search GET /prompts* POST /prompts/render|evaluate` + `feat(frontend): EmailService searchRag/listPrompts + Inbox SearchField ragResults + Home prompt list + i18n` + `feat(electron): IPC rag:search prompts:* DI + preload CJS/MJS` + `chore(state): Fase 5C PROJECT_STATE` (212 tests 91.15%/83.93% verde, `lint`+`format:check`+`typecheck`+`build`+`ui5 build` verdes, `quality`+CodeQL pendiente). Base `develop` en `a1e7a19`, incluye PR #28 `feature/fase5b-prompts` (squash en 5C o merge previo).
...
- Fase 5B Prompts (2026-08-28): `PromptPort`+`PromptError`+`PromptService`+recursos JSON+`golden-dataset.json` (ADR-010) — 202 tests 91.82%/83.94% verde local, incluido en `feature/fase5c-ui-electron` (PR #28 `quality`+CodeQL SUCCESS).
- Fase 5C UI5/Electron (2026-09-03): Backend `createRagRouter` + `createPromptRouter` + `createApp` DI `rag`/`prompts`, Frontend `EmailService` + `Inbox`/`Home` + `i18n`, Electron `ipc.ts`/`preload` DI `rag:search`/`prompts:*` — 212 tests 91.15%/83.93% verde local, `ui5 build` OK, pendiente PR `feature/fase5c-ui-electron` → `develop`.
